...
class MySubmission(Submission):
    ...
    def get_prediction(self, data):
        ...
        return model.predict(data[np.newaxis])[0][0]
        #data = list(pd.Series(data).fillna(0))
    ...
    def run_submission(self):
        ...
        while(True):
            ...
            data = np.array(list(pd.Series(data).fillna(0)))
            if input.shape[0] == 0:
                input = np.zeros((10,60))
                input = input + data
            else:
                input = np.r_[input[1:,:],data.reshape((1,-1))]
